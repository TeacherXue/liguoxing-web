...
class pojieMeal
{
    ...
    {
		// 获取插件信息
		$weapp = Weapp::get(array('code' => 'pojie'));
		// 获取插件配置信息
		$row = json_decode($weapp->data, true);
		if (!empty($row)) {
		    foreach ($row['captcha'] as $key => $val) {
				if ('pojie-plus' == $key && 1 == $val['is_on']) {
					$diestr = array();
					$dierep = array();
					...
					array_push($ecorep, ';if($pojie)tpCache(', ';if($pojie)getUsersConfigData(');
					...
					array_push($logrep, '@(', '"error";if($pojie)$', '@(');
