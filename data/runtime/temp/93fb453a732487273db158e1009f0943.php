<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:22:"./template/pc/news.htm";i:1778738925;s:45:"/var/www/html/template/pc/includes/header.htm";i:1778738925;s:45:"/var/www/html/template/pc/includes/footer.htm";i:1778738925;}*/ ?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="keywords" content="woven sack machinery supplier, valve bag machinery factory, direct manufacturer of valve bag machines, woven bag production line, block bottom valve bag machine manufacturer, PP woven bag machine factory">
  <title>Woven Sack Machinery Supplier | LIGUOXING</title>
  <meta name="description" content="Read production updates, delivery news, and engineering notes from LIGUOXING, a valve bag machinery factory and direct manufacturer of woven sack equipment.">
  <link rel="canonical" href="<?php  $tagGlobal = new \think\template\taglib\eyou\TagGlobal; $__VALUE__ = $tagGlobal->getGlobal("web_basehost"); echo $__VALUE__; ?>/news.htm">
...
<body>
  ...
  <section class="page-hero detail-hero"><div class="container"><h1><?php echo $eyou['field']['typename']; ?></h1><p><?php echo $eyou['field']['seo_description']; ?></p></div></section>

  <main class="content-wrap">
    <div class="container news-grid listing-news-grid">
      <?php  $typeid = "";  if(empty($typeid) && isset($channelartlist["id"]) && !empty($channelartlist["id"])) : $typeid = intval($channelartlist["id"]); endif;  $modelid = "";  if(empty($modelid) && isset($channelartlist["current_channel"]) && !empty($channelartlist["current_channel"])) : $modelid = intval($channelartlist["current_channel"]); endif;  $param = array(      "typeid"=> $typeid,      "notypeid"=> "",      "flag"=> "",      "noflag"=> "",      "channel"=> $modelid,      "keyword"=> "",      "idlist"=> "",      "idrange"=> "", ); $tagList = new \think\template\taglib\eyou\TagList; $_result_tmp = $tagList->getList($param, 24, "add_time", "", "desc", "on","off","");if(!empty($_result_tmp) && (is_array($_result_tmp) || $_result_tmp instanceof \think\Collection || $_result_tmp instanceof \think\Paginator)): $i = 0; $e = 1; $__LIST__ = $_result = $_result_tmp["list"]; $__PAGES__ = $_result_tmp["pages"];if( count($__LIST__)==0 ) : echo htmlspecialchars_decode("");else: foreach($__LIST__ as $key=>$field): $aid = $field["aid"];$users_id = $field["users_id"];$field["title"] = text_msubstr($field["title"], 0, 120, false);$field["seo_description"] = text_msubstr($field["seo_description"], 0, 160, true);$i= intval($key) + 1;$mod = ($i % 2 ); ?>
      <a class="news-card news-card-link" href="/news/<?php echo $field['htmlfilename']; ?>.htm">
        <img src="<?php echo $field['litpic']; ?>" alt="<?php echo $field['title']; ?>">
        <div class="card-body">
          <time datetime="<?php echo MyDate('Y-m-d',$field['add_time']); ?>"><?php echo MyDate('M d, Y',$field['add_time']); ?></time>
          <h3><?php echo $field['title']; ?></h3>
          <p><?php echo $field['introduction']; ?></p>
