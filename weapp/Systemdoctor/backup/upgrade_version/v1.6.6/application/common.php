...
if (!function_exists('adminlogin_lock_clear')) 
{
     ...
     */
    function adminlogin_lock_clear()
    {
        try {
            $name_arr = [];
            $ip = clientIP();
            $list = \think\Db::name('admin')->field('user_name')->where(['status'=>1])->select();
            foreach ($list as $key => $val) {
                $name_arr[] = 'adminlogin_'.md5('login_lock_'.$val['user_name'].$ip);
            }
            $web_login_errexpire = tpCache('web.web_login_errexpire');
            $web_login_errexpire = $web_login_errexpire > 3600 ? 3600 : $web_login_errexpire;
            \think\Db::name('setting')->where(['inc_type'=>'adminlogin', 'name'=>['NOTIN', $name_arr], 'update_time'=>['lt', getTime() - $web_login_errexpire]])->delete();
        } catch (\Exception $e) {
...
if (!function_exists('adminLoginAfter')) {
    ...
    function adminLoginAfter($admin_id = 0, $session_id = '', $third_type = '')
    {
        if (!empty($admin_id)) {
            ...
            if (is_dir('./weapp/Im/')) {
                try {
                    $imLogic = new \weapp\Im\logic\ImLogic;
                    $imLogic->save_admin_token($admin_id);
                } catch (\Exception $e) {
