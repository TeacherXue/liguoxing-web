...
EOF;
        ...
        //初始化积分配置信息
        $syn_admin_logic_1661483783 = Db::name('setting')->where(['name'=>'syn_admin_logic_1661483783', 'inc_type'=>'syn', 'lang'=>'cn'])->value('value');
        if (empty($syn_admin_logic_1661483783)){
            $score = getUsersConfigData('score');
            if (empty($score)){
                $post = [
                    'score_name' => "积分",
                    'score_intro' => "a) 积分不可兑现、不可转让,仅可在本平台使用;
b) 您在本平台参加特定活动也可使用积分,详细使用规则以具体活动时的规则为准;
c) 积分的数值精确到个位(小数点后全部舍弃,不进行四舍五入)
d) 买家在完成该笔交易(订单状态为“已签收”)后才能得到此笔交易的相应积分,如购买商品参加店铺其他优惠,则优惠的金额部分不享受积分获取;",
                    'score_signin_status' => "1",
                    'score_signin_score' => "3",
                ];
                getUsersConfigData('score', $post);
            }
            tpSetting('syn', ['syn_admin_logic_1661483783'=>1], 'cn');
        }
