...
EOF;
    ...
    private function eyou_v167_handle_data()
    {
        ...
        // 商品服务封面图字段长度
        $syn_admin_logic_1716446109 = tpSetting('syn.syn_admin_logic_1716446109', [], 'cn');
        if (empty($syn_admin_logic_1716446109)) {
            try {
                $sql = "ALTER TABLE `{$Prefix}shop_goods_label` MODIFY COLUMN `label_pic`  varchar(250) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '标签路径' AFTER `label_title`;";
                @Db::execute($sql);
                schemaTable('shop_goods_label');
                tpSetting('syn', ['syn_admin_logic_1716446109' => 1], 'cn');
            } catch (\Exception $e) {
            }
        }

        // 消息推送的开关
        $syn_admin_logic_1714458360 = tpSetting('syn.syn_admin_logic_1714458360', [], 'cn');
        if (empty($syn_admin_logic_1714458360)) {
            try {
                $wechat_push_notice_open = 0;
                $adminBindNum = Db::name('admin')->where(['wechat_followed'=>1])->count();
                if (!empty($adminBindNum)) {
                    $wechat_push_notice_open = 1;
                }
                $data = [
                    'wechat_push_notice_open' => $wechat_push_notice_open,
                ];
                /*多语言*/
                if (is_language()) {
                    $langRow = \think\Db::name('language')->order('id asc')
                ...
                tpSetting('syn', ['syn_admin_logic_1714458360' => 1], 'cn');
            } catch (\Exception $e) {
            }
        }
    }
