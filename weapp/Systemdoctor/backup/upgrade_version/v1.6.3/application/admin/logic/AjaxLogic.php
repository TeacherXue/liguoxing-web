...
EOF;
    ...
    public function copy_tplpayfile($channel = 0)
    {
        ...
        if (stristr($dest_path, '/pc/system/')) {
            ...
            if (in_array($channel, [0,2]) && !file_exists($dest_path.'product_comment.htm') && file_exists($source_path.'comment/pc/product_comment.htm')) {
                @copy($source_path.'comment/pc/product_comment.htm', $dest_path.'product_comment.htm');
            }
        }
        ...
        if (file_exists($dest_path)) {
            $dest_path .= '/system/';
            tp_mkdir($dest_path);
            if (in_array($channel, [0,2]) && !file_exists($dest_path.'product_comment.htm') && file_exists($source_path.'comment/mobile/product_comment.htm')) {
                @copy($source_path.'comment/mobile/product_comment.htm', $dest_path.'product_comment.htm');
            }
            if (in_array($channel, [0,2]) && !file_exists($dest_path.'comment_list.htm') && file_exists($source_path.'comment/mobile/comment_list.htm')) {
                @copy($source_path.'comment/mobile/comment_list.htm', $dest_path.'comment_list.htm');
            }
        }
    }
