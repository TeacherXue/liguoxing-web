        ...
        if (is_array($params) && $params['errcode'] == 0 && !$is_internal_network) {
            if (!empty($params['info'])) {
                ...
                if (empty($tpCacheData[$tmpMeal])) {
                    getUsersConfigData('shop', ['shop_open' => 1]);
            ...
            if (empty($params['info']['code'])) {
                ...
                if (is_language()) {
                    $langRow = \think\Db::name('language')->order('id asc')->select();
                    foreach ($langRow as $key => $val) {
                        tpCache('web', [$iseyKey => 0], $val['mark']); // n
                    }
                } else { // 单语言
                    tpCache('web', [$iseyKey => 0]); // n
                }
                /*--end*/
            ...
            try {
                ...
                if (preg_match('/^v(\d+)\.(\d+)\.(\d+)_(.*)$/i', $version)) {
                    ...
                    if (is_language()) {
                        $langRow = \think\Db::name('language')->order('id asc')->select();
                        foreach ($langRow as $key => $val) {
                            tpCache('php', [$tmpMeal => 2, $tmpSerInfo=>$serviceinfo], $val['mark']);
                            tpCache('web', [$iseyKey=>$value], $val['mark']);
                        }
                    } else { // 单语言
                        tpCache('php', [$tmpMeal => 2, $tmpSerInfo=>$serviceinfo]);
