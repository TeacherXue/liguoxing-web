...
class ShopLogic extends Model
{
    ...
    function  __construct() {
        ...
        $this->upgrade_url = $this->service_url . '&domain='.preg_replace('/^(([^\:]+):)?(\/\/)?([^\/\:]*)(.*)$/i','${4}',base64_decode($this->service_ey)).'&v=' . $this->version.'&type=theme_shop&cms_version='.getVersion().'&ip='.serverIP();
