            ...
            if($result){
                ...
                foreach ($data as $k=>$v){
                    ...
                    if(!isset($temp[$newK])){
                        ...
                        if($v == -1 && $newK == 'web_is_authortoken') { continue; }if(strval($v)!==strval($temp[$newK])){
                            $table_db->where([
                                'name'  => $newK,
                                'lang'  => $lang,
...
if (!function_exists('handle_subdir_pic')) 
{
    ...
    function handle_subdir_pic($str = '', $type = 'img', $domain = false, $clear_root_dir = false)
    {
                ...
                if (!empty($imgArr)) {
                    ...
                    foreach ($imgArr as $key => $value) {
                        ...
                        if (isset($img_val[2][0]) && !is_http_url($img_val[2][0])) { // 是否本地图片
                            ...
                            $handle_img = preg_replace('#(/[/\w\-]+)?(/public/upload/|/public/static/|/uploads/|/weapp/)#i', $domainStr.$add_root_dir.'$2', $value);
        ...
        if (!empty($str) && !is_http_url($str) && !in_array($type, ['html'])) {
            if (false !== $domain) {
                if (true === $domain) {
        ...
        if (empty($orderData)) {
            ...
            if (false !== $ret) {
                ...
                if (!empty($orderData['points_shop_order'])) {
                    ...
                    $detailsData = !empty($detailsData) ? safe_unserialize($detailsData) : [];
    ...
    function usershomeurl($users_id)
    {
        ...
        if (is_dir('./weapp/Users/') && null === $is_users_weapp) {
            ...
            if (!empty($weappInfo['status'])) {
                $is_users_weapp = true;
                $weappInfo['data'] = safe_unserialize($weappInfo['data']);
                if (false === $weappInfo['data']) {
                    // 处理错误
                    $weappInfo['data'] = [];
                }
        ...
        if (!empty($admin_id)) {
            ...
            if (0 < intval($role_id)) {
                $auth_role_info = \think\Db::name('auth_role')
                    ...
                    ->find();
                if (!empty($auth_role_info)) {
                    $auth_role_info['language'] = safe_unserialize($auth_role_info['language']);
                    $auth_role_info['cud'] = safe_unserialize($auth_role_info['cud']);
                    $auth_role_info['permission'] = safe_unserialize($auth_role_info['permission']);
        ...
        if (is_dir('./weapp/Ask/')){
            try{
                ...
                if (!empty($Askow['status']) && 1 == $Askow['status']) {
                    ...
                    $ask_seo_pseudo = 1;
                    $Askow['data'] = safe_unserialize($Askow['data']);
    ...
    function auto_bind_wechatlogin($users = [], &$referurl = '')
    {
        ...
        if (isMobile() && isWeixin() && 0 === intval($users['thirdparty'])) {
            ...
            if (empty($open_id)) {
                ...
                $weChatLoginConfig = !empty($weChatLoginConfig) ? safe_unserialize($weChatLoginConfig) : [];
            ...
            //         $WeChatLoginConfig = !empty($wechat_login_config) ? safe_unserialize($wechat_login_config) : [];
            //         // 微信授权登陆
...
if (!function_exists('get_weixin_access_token')) 
{
    ...
    function get_weixin_access_token($resetToken = false, $applets = 'openSource')
    {
        if (is_dir('./weapp/DiyminiproMall/') && 'visualization' == $applets) {
            ...
            if (!empty($data)) {
                 $decodedData = safe_unserialize($data);    
